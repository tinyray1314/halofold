...
- 日常功能经你授权后只读访问本机 `~/.codex` 中的任务状态、标题和对话结果；不会读取、复制或保存 Codex 登录凭据。
- 仅当你主动启动“修复会话历史”并经过写入授权与最终确认后，应用才会在 Codex 已退出的前提下创建本地快照，并只向侧边栏目录新增符合严格条件的缺失入口；绝不删除或覆盖记录，也不修改会话主库、rollout、账号或 provider 配置。该写入授权不会长期保存。
